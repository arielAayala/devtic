...
class Demandas {
    ...
    public function obtenerDemandasPorFiltros($token, $pagina, $idEstado, $idTipo, $idOrganizaciones, $fechaIngreso, $fechaCierre) {
        if ($datos = Profesionales::validarToken($token)) {
            $con = new Conexion();
            $query = "SELECT d.idDemanda, p.fotoProfesional, especialidades.nombreEspecialidad, personas.nombrePersona, d.motivoDemanda, d.fechaIngresoDemanda, d.relatoDemanda, e.nombreEstado, t.nombreTipo, o.nombreOrganizacion
                    FROM demandas d
                    INNER JOIN estados e ON e.idEstado = d.idEstado
                    INNER JOIN tipos t ON d.idTipo = t.idTipo
                    INNER JOIN organizaciones o ON o.idOrganizacion = d.idOrganizacion
                    ...
                    INNER JOIN personas ON personas.idPersona = p.idPersona
                    INNER JOIN especialidades ON especialidades.idEspecialidad = p.idEspecialidad
                    WHERE d.borrarDemanda = 0";
    
            // Aplicar filtros
            if (!empty($idEstado)) {
                $query .= " AND d.idEstado = $idEstado";
            }
    
            if (!empty($idTipo)) {
                $query .= " AND d.idTipo = $idTipo";
            }
    
            if (!empty($idOrganizaciones)) {
                $query .= " AND d.idOrganizacion = $idOrganizaciones";
            }
    
            if (!empty($fechaIngreso)) {
                $query .= " AND d.fechaIngresoDemanda >= '$fechaIngreso'";
            }
    
            if (!empty($fechaCierre)) {
                $query .= " AND d.fechacieredemanda <= '$fechaCierre'";
            }
    
            $query .= " ORDER BY d.fechaIngresoDemanda DESC LIMIT 10 OFFSET " . (($pagina - 1) * 10);
            ...
            $queryTotal = "SELECT COUNT(idDemanda) AS demandasTotales FROM demandas WHERE borrarDemanda = 0";
            if (!empty($idEstado)) {
                $queryTotal .= " AND idEstado = $idEstado";
            }
    
            if (!empty($idTipo)) {
                $queryTotal .= " AND idTipo = $idTipo";
            }
    
            if (!empty($idOrganizaciones)) {
                $queryTotal .= " AND idOrganizacion = $idOrganizaciones";
            }
    
            if (!empty($fechaIngreso)) {
                $queryTotal .= " AND fechaIngresoDemanda >= '$fechaIngreso'";
            }
    
            if (!empty($fechaCierre)) {
                $queryTotal .= " AND fechaCierredemanda <= '$fechaCierre'";
            }
            if ($resultadoTotal = $con->query($queryTotal)) {
                while ($row = $resultadoTotal->fetch_assoc()) {
                    $total = $row["demandasTotales"];
